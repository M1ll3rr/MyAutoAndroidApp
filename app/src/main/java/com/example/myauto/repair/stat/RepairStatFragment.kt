...
import com.example.myauto.databinding.FragmentRepairStatBinding
import com.example.myauto.repair.RepairViewModelFactory
...
class RepairStatFragment : BaseStatFragment<DailyCost, StatSummary>() {
    private val binding: FragmentRepairStatBinding by viewBinding(
        FragmentRepairStatBinding::bind
    )
    override val viewModel: RepairStatViewModel by viewModels {
        RepairViewModelFactory(
            (requireActivity() as MainActivity).getRepository()
        )
    }
    override val layoutResId = R.layout.fragment_repair_stat
